...
export class TicketRestService {
  ...
  getTours(): Observable<ITour[]> {
    // return this.http.get<ITour[]>('http://localhost:4200/tours');
  ...
  sendTourData(data: IOrder): Observable<IOrder> {
    return this.http.post<IOrder>('http://localhost:4200/order', data);
  }

  createTour(body: any): Observable<any> {
    return this.http.post('http://localhost:4200/tour-item', body, { headers: {
